...
def send_activation_email(to_email_id, token):
    ...
    html_message = html_message = render_to_string('Login/verification_email_template.html', {'context': token})
...
def send_reset_email(to_email_id, token):
    ...
    html_message = html_message = render_to_string('Login/reset_password_email_template.html', {'context': token})
    plain_message = strip_tags(html_message)
    from_email = settings.EMAIL_HOST_USER
    to = to_email_id
    send_mail(subject, plain_message, from_email, [to], html_message = html_message,fail_silently=True)